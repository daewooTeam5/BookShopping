...
public interface ReviewAdminRepository {
    ...
    @Select("SELECT r.id, r.account_id, r.book_id, r.rating, r.content, b.title AS bookTitle " +
            "FROM review r " +
    ...
    @Select(
    	      ...
    	      "         ROUND(AVG(TO_NUMBER(r.rating)), 2) AS avgRating," +
    	      "         COUNT(*) AS reviewCount " +
    	      "  FROM review r " +
    	      "  JOIN book b ON r.book_id = b.id " +
    	      "  GROUP BY b.id, b.title " +
    	      "  ORDER BY AVG(TO_NUMBER(r.rating)) DESC, COUNT(*) DESC" +
    	      ") WHERE ROWNUM <= 5"
    	    )
